...
LET IN END BACKSLASH DOT DEF SEMICOLON PARALLEL LOCAL EOF COLON TBOOL TINT ARROW
...
expression:
  |expression2 expression {FunctionCall($1,$2)}
  |expression2 {$1}

expression2:
  |BACKSLASH ID COLON mytype DOT expression1 {FunctionAbstraction(($2,$4),$6)}
  |expression1 {$1}
  
expression1:
  expression1 DISJ conj_expression {Disjunction ($1,$3)}
  | conj_expression {$1}
  | let_exp {$1}
...
sim_definition:
  |DEF ID COLON mytype EQ expression  {Simple(($2,$4),$6)}
  |DEF ID EQ expression {Simple_untyped ($2,$4)}  
...
mytype:
  |type_terminal {$1}
  |mytype ARROW type_terminal {Tfunc($1,$3)}
...
typel_1:
  |LP mytype TIMES mytype {[$2;$4]}
  |typel_1 TIMES mytype {$1 @ [$3]}
;