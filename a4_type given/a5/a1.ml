...
type opcode = VAR of string | NCONST of bigint | BCONST of bool | ABS | UNARYMINUS | NOT
  ...
  | PAREN | IFTE of (opcode list * opcode list )| TUPLE of int | PROJ of int*int | LET | FABS of string * (opcode list) | FCALL | RET
  | SIMPLEDEF of string | SEQCOMPOSE | PARCOMPOSE | LOCALDEF


(* The language should contain the following types of expressions:  integers and booleans *)
type answer = Num of bigint | Bool of bool | Tup of int * (answer list) | Fun of (string * answer) | VClos of (opcode list) * (string  * answer) list
type value = NumVal of int | BoolVal of bool | TupVal of int * (value list) | FunVal of string  * value | Vclosure of exptree * (string * value) list


(* call by name  *)
type  closure = 
  Clos of exptree *(string * closure) list
...
let rec compl ex ll  = match ex with 
  ...
  | IfThenElse (x,y,z) -> compl x ((IFTE(compl y [],compl z []))::ll)
  ...
  | Let (d,e) -> 
    let rec compld d ll = begin
    match d with 
      Simple((s,_),e1)|Simple_untyped(s,e1)-> (compl e1 (SIMPLEDEF(s)::ll)) 
      |Sequence (dl) ->
       let rec compldsl dl ll = 
        match dl with [] -> SEQCOMPOSE::ll
        | dlx::dlxs -> compld dlx (compldsl dlxs ll)
      in compldsl dl ll
      |Parallel (dl) -> let rec compldsl dl ll = 
        match dl with [] -> PARCOMPOSE::ll
        | dlx::dlxs -> compld dlx (compldsl dlxs ll)
      in compldsl dl ll
      |Local(d1,d2) -> compld d1 (compld d2 (LOCALDEF::ll))
    end in compl e (compld d (LET::ll))
  | FunctionAbstraction ((s,t),e) -> FABS(s,compl e (RET::[]))::ll
  | FunctionCall (e1,e2) -> compl e1 (compl e2 (FCALL::ll))
...
match px with 
    ...
    | IFTE (y,z)  -> begin
                  match stk with (a::(stks)) ->
                      begin 
                    match (a) with (Bool x) -> if x then stkmc stks rho (List.append y pxs)
                                                else stkmc stks rho (List.append z pxs)                                             
                    | _ -> raise IllFormedOCL 
                    end 
    ...
    | FABS (s,ocl) -> (match stk with (a::stks) ->
                      stkmc ((Fun (s,a))::stks) rho pxs
                    |_-> raise IllFormedOCL) 
