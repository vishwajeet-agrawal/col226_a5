...
let rec hastype1 c t  = 
  ...
  | Var x -> 
    ...
    let(a,b,c) = hastype1 c1 t in
    if a then (true,(x,t)::gt,removekeys gut [(x,0)]) else (false,[],[])
    else (false,[],[])
        ...
        begin
          ...
          | Var x -> if exists_l gt1 x then begin
             ...
             | _-> (false,[],[])
              ...
              let(a,b,c) = hastype2 (extract_l gut1 x) t fex in
              if a then (true,gt1,gut1) else (false,[],[])
            end
            else raise Insufficient_TA
          ...
          | FunctionCall(x,y) ->
            hastype2 (Tclos(x,gt1,gut1)) t (FunctionCall(y,fex)) 
            
          | FunctionAbstraction(s,x) ->
            let gt11 = if exists_l gt1 s then removekeys gt1 [(s,0)] else gt1 in
            let(a,b,c)=hastype1 (Tclos(x,gt11,(s,Tclos(fex,gt1,gut1))::gut1)) t in
